...
;(async () => {
  try {
    const client = await MongoClient.connect(process.env.MFLIX_DB_URI, { useNewUrlParser: true })
    ...
    const cursor = await moviesCollection
    ...
    const { modifiedCount } = await moviesCollection.bulkWrite(moviesToMigrate)
